...
import OfflineRequestRecharge from "../models/OfflineRecharge.js";
...
/**
 *  Submit Offline Recharge Request
 */
export const submitOfflineRequest = asyncHandler(async (req, res) => {
  const { amount, bank, utr, date, mode } = req.body;

  if (!amount || !bank || !utr || !date || !mode) {
    return res.status(400).json({ ok: false, message: "All fields are required" });
  }

  // Check if UTR already exists
  const existing = await OfflineRequestRecharge.findOne({ utr });
  if (existing) {
    return res.status(400).json({ ok: false, message: "UTR already submitted" });
  }

  const request = await OfflineRequestRecharge.create({
  ...
  res.status(201).json({ ok: true, message: "Request submitted successfully", request });
});

/**
 * 📋 Get All Offline Requests (Admin)
 */
export const getAllOfflineRequests = asyncHandler(async (req, res) => {
  const requests = await OfflineRequestRecharge.find()
...
/**
 *  Approve/Reject Offline Request (Admin)
 */
export const processOfflineRequest = asyncHandler(async (req, res) => {
  const { requestId, status, remarks } = req.body; // status: 'approved' | 'rejected'

  const request = await OfflineRequestRecharge.findById(requestId);
  if (!request) {
  ...
  if (status === "approved") {
    // Credit Wallet
    await creditWalletAndLogTransaction(
      request.retailerId,
      request.amount,
      // `OFFLINE_${request._id}`  // Original meta format
      `Offline Recharge Request Credit: UTR-${request.utr}`  // Use UTR for better traceability
    );
  ...
  res.json({ ok: true, message: `Request ${status} successfully`, request });
});

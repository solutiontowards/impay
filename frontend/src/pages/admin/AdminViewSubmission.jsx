...
const ComplaintStatusBadge = ({ status }) => (
  <span
    className={`px-2.5 py-1 text-xs font-semibold rounded-full capitalize ${status === 'Pending' ? 'bg-yellow-100 text-yellow-800' :
...
const FileCard = ({ fileName, fileUrl }) => {
  ...
  const handleDownload = () => {
    ...
    try {
      ...
      a.href = fileUrl;               // Direct S3 URL → no CORS required
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              ...
              <DetailItem
                ...
                value={submission.retailerId.mobile}
              />
              <DetailItem
                icon={Tag}
                label="Service Name"
